...
fn assert_valid_svd(a: &ArrayBase<impl Data<Elem = f32>, Ix2>) {
    ...
    let (u, singular_values, vt) = backend.compute_svd(a).expect("Julia f32 SVD failed");
...
fn benchmark_repeated_svd() {
    ...
    backend.compute_svd(&a).expect("Julia f32 warm-up SVD failed");
    let first_elapsed = first_start.elapsed();

    let repeated_start = Instant::now();
    for _ in 0..REPEATS {
        backend.compute_svd(&a).expect("Julia f32 repeated SVD failed");
    }
