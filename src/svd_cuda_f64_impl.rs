...
impl SvdBackend<f64> for CudaF64Svd {
    fn compute_svd(
        ...
        let d_a = DeviceBuffer::new(checked_bytes::<f64>(elements_a, "A")?, "A")?;
        let d_s = DeviceBuffer::new(checked_bytes::<f64>(k, "singular values")?, "singular values")?;
        let d_u = DeviceBuffer::new(checked_bytes::<f64>(elements_u, "U")?, "U")?;
        let d_vt = DeviceBuffer::new(checked_bytes::<f64>(elements_vt, "Vt")?, "Vt")?;
