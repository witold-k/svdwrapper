...
impl SvdBackend<f32> for CudaF32Svd {
    fn compute_svd(
        ...
        let mut a_col = vec![0.0f32; (m * n) as usize];
        for r in 0..a.nrows() {
            for c in 0..a.ncols() {
        ...
        let bytes_s = k as usize * elem_size; // FIX 3: Min(M,N) nutzen
        ...
        unsafe {
            ...
            let mut lwork: i32 = 0;
            let stat = cusolverDnSgesvd_bufferSize(self.handle, m, n, &mut lwork);
            if stat != cusolverStatus_t::CUSOLVER_STATUS_SUCCESS {
                cuMemFree_v2(d_a); cuMemFree_v2(d_s); cuMemFree_v2(d_u); cuMemFree_v2(d_vt);
                return Err(anyhow!("cusolverDnSgesvd_bufferSize failed: {:?}", stat));
            }
            ...
            let stat = cusolverDnSgesvd(
                self.handle, b'A' as i8, b'A' as i8, m, n,
                d_a as *mut f32, lda, d_s as *mut f32, d_u as *mut f32, m,
                d_vt as *mut f32, n, d_work as *mut f32, lwork,
            ...
            if stat != cusolverStatus_t::CUSOLVER_STATUS_SUCCESS {
                ...
                return Err(anyhow!("cusolverDnSgesvd failed: {:?}", stat));
            }

            let mut s_vec = vec![0f32; k as usize];
            let mut u_col_res = vec![0f32; (m * m) as usize];
            let mut vt_col_res = vec![0f32; (n * n) as usize];
